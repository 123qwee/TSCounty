import five from '../../templates/module/module.static.five.html';
import service from "../services/service.js";

export class staticFive {
  init() {
    let that = this;
    $(".page-5-1").empty().append(five);
    that.getData();
    var fiveTime = that.random(60000, 180000);
    var dataInterval = setInterval(function () {
      that.getData();
    }, fiveTime);
    console.log("重点人群饼图获取数据时间", fiveTime);
    // 在1分钟至3分钟内任取时间
...
*/
  ...
  createBars(data) {
    ...
    var that = this;
    if (window.fiveTime) {
      _.map(fiveInterval, item => {
        window.clearInterval(item);
      })
    } else {
      window.fiveInterval = [];
      window.fiveTime = false;
    }
    fiveInterval.push(setInterval(function () {
      fiveTime = true;
